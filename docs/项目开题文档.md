# XJU ICTHub 实验室项目管理平台开题文档

| 项目项 | 内容 |
| --- | --- |
| 项目名称 | XJU ICTHub 实验室项目资源库与项目知识管理平台 |
...
ICTHub 的目标不是重新实现 Jira，而是建立一套适合实验室长期维护的“项目档案库 + 资源索引 + 轻量管理工作台”。
...
负责人、研发状态、资源列表、技术标签与锐评在进入详情后展示，避免首页成为字段堆叠的管理看板。
...
| 实验室管理员 | 批量导入、合并重复项目、管理权限、检查失效资源 |
...
### 7.2 对 `xju-feiyue` 的设计参考

本项目参考 `XjuSelab/xju-feiyue` 的前端设计体系，但不直接复制其业务代码：
...
| 身份认证 | 首期 Session Cookie 或短期 JWT + Refresh Token |
...
建议保留单体应用，不在首期拆分微服务。项目、资源、导入、认证和 Agent Gateway 可以放在同一个 Rust Workspace 中按 crate 或 module 分层。
...
解析器识别 GitHub、百度网盘、普通 URL 和文件名扩展名，生成资源草稿。用户确认后才提交。

### 10.2 表格导入

- 支持 CSV，Excel 可由前端解析为统一表格预览；
- 明确列映射：项目名、类别、简介、曾获奖、负责人、状态、资源链接；
- 提交前显示新增、更新、跳过和冲突数量；
- 导入操作必须生成审计记录并支持查看失败原因。
...
- 所有写接口进行权限校验；
- 使用 HTTP-only、Secure、SameSite Cookie 或经过审查的 Token 方案；
- 上传文件限制类型、大小与数量；
...
- React 构建产物由反向代理直接提供；
- Rust API 以单实例起步，设置合理连接池；
- PostgreSQL 与附件存储使用持久卷；
- 首期不部署 Kubernetes、Elasticsearch、Kafka 和独立向量数据库；
- 搜索先使用 PostgreSQL 全文检索与 `pg_trgm`；
- `icthub.top` 启用 HTTPS、HSTS 和自动证书更新；
- GitHub Actions 执行前后端测试与镜像构建，部署需人工批准。
...
| P1 前后端骨架 | 第 2 周 | React 应用壳、Design Tokens、Axum 服务、数据库迁移、CI |
| P2 项目与奖项 | 第 3 周 | 项目 CRUD、类别筛选、搜索、项目详情、曾获奖维护 |
| P3 资源与导入 | 第 4 周 | 资源 CRUD、上传、混合文本导入、CSV 导入预览 |
| P4 权限与 Agent | 第 5 周 | 登录、角色权限、审计、只读 Agent 问答 |
| P5 验收与部署 | 第 6 周 | 自动化测试、备份恢复演练、性能检查、icthub.top 部署 |
...
- 可以创建、编辑、搜索、筛选和归档项目；
- 首页稳定展示项目类别、名称、简介和曾获奖；
- 项目详情可以维护负责人、状态、锐评、标签、奖项和资源；
- 可以导入包含 GitHub、百度网盘和文件名称的混合文本；
- 可以预览 CSV 导入结果并确认提交；
- 权限不足的用户无法调用写接口；
...
| 首页再次变复杂 | 固定首页四项信息，新增字段默认进入详情 |
| Agent 产生错误结论 | 返回引用、首期只读、写操作必须人工确认 |
| 4C8G 资源不足 | 单体架构、PostgreSQL 搜索、外部模型 API、避免重型中间件 |
...
1. `icthub.top` 是公开项目门户，还是默认需要登录；
2. 百度网盘首期只记录链接，还是需要人工确认可用性流程；
3. 上传文件首期容量和单文件大小限制；
4. 用户认证采用学校统一身份、GitHub OAuth 还是内部账户；
5. 项目主类别是否允许管理员自定义；
6. “曾获奖”是否只记录最高奖项，还是首页显示最高奖、详情记录全部奖项；
7. Agent 使用的模型供应商与预算；
8. 是否需要把 `xju-feiyue` 的 Design Tokens 抽成共享包，还是在本项目独立维护。

## 18. 开题结论

XJU ICTHub 的核心价值是把分散的项目文件转化为可理解、可交接、可继续利用的项目资产。首期应坚持简约首页、结构化档案、快速导入和轻量 Rust 单体架构，不应过早扩展为复杂项目管理系统。

开题阶段建议立即推进：

1. 确认待确认事项中的访问权限、存储限制和认证方式；
2. 按 `xju-feiyue` 的 Design Tokens 与组件分层初始化 React 前端；
3. 初始化 Rust Workspace、Axum、SQLx 与首批数据库迁移；
