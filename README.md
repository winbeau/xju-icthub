...
项目已经进入实现阶段。React/Rust 工程骨架、飞跃登录复用、简约项目列表与详情、项目创建/编辑/归档、资源维护、表格快速导入、SQLite 迁移和身份适配器已经落地。一键导入现已具备持久化 Worker、Office/PDF/媒体抽取、清洗后的分析包，以及固定版本 Codex CLI 的结构化 Agent 运行器；真实模型调用需在服务器配置 Base URL、模型名和受限权限的 Codex 认证目录后开启。
