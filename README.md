...
当前首期建设内容是其中的“项目库”模块，用于统一整理实验室历届与在研项目，记录项目类别、内容简介、曾获奖、负责人、状态与相关资源，并提供 Codex 一键导入、检索和 Agent 问答能力。
...
项目已经进入实现阶段。React/Rust 工程骨架、飞跃登录复用、简约项目列表与详情、项目创建/编辑/归档、资源维护、SQLite 迁移和身份适配器已经落地。一键导入现已具备持久化 Worker、Office/PDF/媒体抽取、清洗后的分析包，以及固定版本 Codex CLI 的结构化 Agent 运行器；真实模型调用需在服务器配置 Base URL、模型名和受限权限的 Codex 认证目录后开启。
